...
use super::repos::{
    AuthRequestRepo, AuthVerificationRepo, InteractionRepo, IssuingRepo, MinionsRepo
};
...
impl RepoTrait for RepoForSql {
    fn request(&self) -> Arc<dyn RequestRepoTrait> { self.request_repo.clone() }

    fn interaction(&self) -> Arc<dyn InteractionRepoTrait> { self.interaction_repo.clone() }

    fn verification(&self) -> Arc<dyn VerificationRepoTrait> { self.verification_repo.clone() }
    fn minions(&self) -> Arc<dyn MinionsRepoTrait> { self.minions_repo.clone() }

    fn issuing(&self) -> Arc<dyn IssuingRepoTrait> { self.issuing_repo.clone() }
}
